...
- 状态：已接受（2026-09-17 Owner 评审通过，含业界标尺修订两轮）
...
## 实施附记（2026-09-17，#2 落地后补记，设计文本不动）

B 路落地时 probe 实证出三个设计稿未预见的问题，实施形态相应调整（详见
[w13-hybrid-eval](../eval/retrieval/w13-hybrid-eval.md)「实施期实证发现」）：

1. **摄入/查询双侧文本规整化**（`FullTextNormalizer`）：multilingual 分词器粘连数字与中文，
   不规整化则代号类查询（B 路的核心靶子）零命中——B 路成立的前置条件，设计稿未提
2. **命中裁决移至应用侧**（`SparseCoverage` 子串覆盖率）：MatchText 严格 AND 无法服务
   自然语言问句，服务端只做候选网——比设计稿"MatchText 捞字面命中"多一层应用侧逻辑，
   但仍在 B 的工程量量级内（无 IDF 自维护）
3. **标识符兜底全扫**：纯数字单 token 被分词器丢弃 + 高频词可洪泛候选网——
   小语料适配解法，语料上量后与候选 A 触发条件一并重评

裁决数据：Recall@5 72%→93%（+21pp ≥ 5pp），负例拒答率零回归，hybrid 默认启用（`5f31315`）。
