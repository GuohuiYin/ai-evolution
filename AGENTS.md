...
- **A9** DDD 功能域分包：按业务功能域组织包结构（现有 `chat`/`analysis`/`rag`/`stock`/`tool`/`eval`/`prompt` 即先例），禁止按技术分层横切建包（如 `controller/`/`service/`/`dao/`），也禁止域内按技术类型再横切（如 `api/`/`model/`/`exception/`）——类型信息由命名后缀（XxxController/XxxService/XxxException）承担，包只表达"什么会一起变"；**新能力从设计之初即按子域独立建包，不等域内文件过载后再被动拆分**（Owner 2026-09-05 明确要求，见 ADR-0003）；域内默认扁平；新能力归入其所属领域包，跨域引用只依赖对方暴露的接口与数据契约；包间依赖方向由 ArchUnit 守护（B2，W5 启用）
