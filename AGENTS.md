...
> 制定日期：2026-09-02 · 版本 v1.2（2026-09-03 新增 A8） · 修改需经仓库所有者确认
...
- **A8** README 三层活文档：项目初见（首个可运行骨架）即建 `README.md`，按三层构建——**稳定层**（项目动机/目标/红线/技术选型/快速上手，一次写全）+ **快照层**（当前架构图与能力清单，标注日期）+ **指针层**（导航 docs/ 各目录，单一事实源，不留悬空链接）；快照层伴随每个里程碑（M1/M2/M3）实时刷新，稳定层与指针层随结构性变更同步更新——保证文档与项目进展对齐不腐烂
